...
$following = User::getFollowingList($pdo, $user_id);
...
<div class="user-list-container">
    <h2>Seguindo por <?= htmlspecialchars($profile_user_data['username']) ?></h2>

    <?php if (!empty($following)): ?>
        <?php foreach ($following as $followed_user): ?>
            <div class="user-list-item">
                <img src="<?= UPLOAD_URL . htmlspecialchars($followed_user['profile_picture'] ?? 'default_profile.png') ?>" alt="Foto de perfil de <?= htmlspecialchars($followed_user['username']) ?>">
                <div class="user-info">
                    <a href="<?= BASE_URL ?>profile.php?id=<?= htmlspecialchars($followed_user['id']) ?>"><?= htmlspecialchars($followed_user['username']) ?></a>
                </div>
                <?php if ($current_user_id && $current_user_id != $followed_user['id']): ?>
                    <?php
                    $is_following_this_user = User::isFollowing($pdo, $current_user_id, $followed_user['id']);
                    ?>
                    <form action="" method="POST" style="display:inline;">
                        <input type="hidden" name="action" value="<?= $is_following_this_user ? 'unfollow' : 'follow' ?>">
                        <input type="hidden" name="target_user_id" value="<?= htmlspecialchars($followed_user['id']) ?>">
                        <input type="hidden" name="redirect_url" value="<?= htmlspecialchars($_SERVER['REQUEST_URI']) ?>">
                        <button type="submit" class="follow-button <?= $is_following_this_user ? 'btn-danger' : 'btn-primary' ?>">
                            <?= $is_following_this_user ? 'Deixar de Seguir' : 'Seguir' ?>
                        </button>
                    </form>
                <?php endif; ?>
            </div>
        <?php endforeach; ?>
    <?php else: ?>
        <p>Este usuário não está seguindo ninguém.</p>
    <?php endif; ?>
</div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['target_user_id'])) {
    ...
    $redirect_url = $_POST['redirect_url'] ?? BASE_URL . 'following.php?id=' . $user_id;
