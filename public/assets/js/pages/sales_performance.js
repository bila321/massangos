...
(function () {
    ...
    const labels = window.SP_CHART.labels;
    const revenue = window.SP_CHART.revenue;
    const counts = window.SP_CHART.counts;
                    ...
                    callbacks: {
                        label: (ctx) =>
                            ctx.dataset.yAxisID === 'y'
                                ? ` ${ctx.raw.toLocaleString('pt-MZ', { minimumFractionDigits: 2 })} MZN`
                                : ` ${ctx.raw} venda${ctx.raw !== 1 ? 's' : ''}`,
                    },
                },
            },
            scales: {
